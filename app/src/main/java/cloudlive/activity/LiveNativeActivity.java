...
import cloudlive.entity.ChatDisableAllStatusEntity;
import cloudlive.entity.Event;
import cloudlive.entity.SignEndEntity;
import cloudlive.entity.SignEntity;
import cloudlive.event.OnSendFlowerListener;
import cloudlive.event.OnSendMessageListener;
import cloudlive.helper.LiveLotteryDialogHelper;
import cloudlive.helper.LiveNetHelper;
import cloudlive.helper.LiveRollHelper;
import cloudlive.helper.LiveSignDialogHelper;
import cloudlive.helper.LiveVoteDialogHelper;
...
import cloudlive.view.GuideDialog;
import cloudlive.view.LiveMessageView;
import cloudlive.view.SoftKeyboardStateWatcher;
...
//        ScreenSwitchUtils.getInstance(this).isOpenSwitchAuto(true);
            ...
            public void call(Object... args) {
                ...
                for (int i = 0; i < args.length; i++) {
                    ...
                    if (mLiveSignDialogHelper != null) {
                        LiveNativeActivity.this.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
    ...
    @OnClick({
            ...
            R.id.video_visibility_iv, R.id.ppt_Layout, R.id.network_choice_iv, R.id.iv_danmu_switch, R.id.iv_refresh/*, R.id.btn_guide_start*/})
    void onClick(View v) {
        switch (v.getId()) {
    ...
    public void onLaunch() {
        if (mHtSdk != null) {
            if (LiveStatus.STOP.equals(mHtSdk.getInitLiveStatus())) {
                ...
                moduleConfig = mHtSdk.getModuleConfig();
            }
...
*/
    ...
    public void updateMemberTotal(int total) {
        if (moduleConfig != null) {
            String memberFloatTVEnable = moduleConfig.getModVisitorinfoLive().getEnable();//控制是否显示人数
            if (!TextUtils.isEmpty(memberFloatTVEnable)) {
                if (memberFloatTV != null) {
